...
func run() error {
	...
	regularFont := flag.String("regular-font", godex.DefaultNotoSansRegular, "regular TTF font path")
	boldFont := flag.String("bold-font", godex.DefaultNotoSansBold, "bold TTF font path")
	flag.Parse()
	...
	case *qr:
		payload := *text
		if payload == "TEST" {
			generated, err := godex.RandomQRPayload()
			if err != nil {
				return err
			}
			payload = generated
